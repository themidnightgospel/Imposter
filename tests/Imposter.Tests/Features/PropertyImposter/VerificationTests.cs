...
namespace Imposter.Tests.Features.PropertyImposter
{
    public class VerificationTests
    {
        ...
        public void GivenPropertyInteractions_WhenVerifyingIncorrectGetterCount_ShouldThrowException()
        {
            ...
            var exception = Should.Throw<Exception>(() => _sut.Age.Getter().Called(Count.Exactly(2)));
            exception.Message.ShouldContain("Invocation was expected to be performed exactly 2 time(s) but instead was performed 1 times.");
        }
        ...
        public void GivenPropertySetterCalls_WhenVerifyingWithMatchingCriteria_ShouldVerifyCorrectCount()
        {
            ...
            Should.NotThrow(() => _sut.Age.Setter(Arg<int>.Is(x => x == 20)).Called(Count.Exactly(1)));
            Should.NotThrow(() => _sut.Age.Setter(Arg<int>.Is(x => x == 10)).Called(Count.Exactly(2)));
