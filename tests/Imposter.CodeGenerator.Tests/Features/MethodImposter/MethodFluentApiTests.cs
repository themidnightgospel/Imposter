...
""");
    ...
    private static GeneratorRunResult RunGenerator()
    {
        var compilation = CSharpCompilation.Create(
            assemblyName: "MethodFluentApiTests",
    ...
    private static ImmutableArray<Diagnostic> CompileSnippet(string snippet)
    {
        ...
        var compilation = CSharpCompilation.Create(
            assemblyName: "MethodFluentApiTests.Snippet",
    ...
    {
        diagnostics.Length.ShouldBe(1, $"Diagnostics:{Environment.NewLine}{string.Join(Environment.NewLine, diagnostics.Select(d => $"{d.Id}: {d.GetMessage()} ({d.Location.GetLineSpan()})"))}");
