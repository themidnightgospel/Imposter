...
public class ImposterGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateImposterAttribute = "Imposter.Abstractions.GenerateImposterAttribute";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(context
                ...
                .ForAttributeWithMetadataName(
                    ...
                    transform: static (ctx, _) => GetImposterTargetTypeSymbol(ctx))
                .Where(static target => target is not null)
                .Select((symbol, token) => (symbol, token)),
            static (spc, target) => Generate(target.symbol!, spc, target.token));
    }

    private static ITypeSymbol? GetImposterTargetTypeSymbol(GeneratorAttributeSyntaxContext context)
    {
    ...
    private static void Generate(ITypeSymbol typeSymbol, SourceProductionContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var imposterGenerationContext = new ImposterGenerationContext(new ImposterTarget((INamedTypeSymbol)typeSymbol), new StringBuilder());
            var sourceText = GenerateImposter(imposterGenerationContext);
            context.AddSource($"{imposterGenerationContext.Target.ImposterClass.Name}.g.cs", SourceText.From(sourceText, Encoding.UTF8));
    ...
    private static string GenerateImposter(ImposterGenerationContext imposterGenerationContext)
    {
        ...
        foreach (var method in imposterGenerationContext.Target.Methods)
        {
            ...
            AddMethodInvocationHistoryClass(imposterGenerationContext, method);
    ...
    private static void AddMethodInvocationHistoryClass(ImposterGenerationContext imposterGenerationContext, ImposterTargetMethod method)
    {
    ...
    private static void AddMethodInvocationVerifierClass(ImposterGenerationContext imposterGenerationContext, ImposterTargetMethod method)
    {
        ...
        if (method.ParametersExceptOut.Count > 0)
        {
            sb.AppendLine($$"""
                                ...
                                {
                                    return {{string.Join(" && ", method.ParametersExceptOut.Select(parameter => $"{parameter.EnclosedInArgNameDeclaredAsField}.Predicate(arguments.{parameter.Symbol.Name})"))}};
                                }
                            """);
        }
