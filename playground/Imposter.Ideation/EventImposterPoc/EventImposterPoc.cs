...
namespace Imposter.Ideation.EventImposterPoc
{
    ...
    public interface ISomethingHappenedEventImposterBuilder
    {
        ...
        ISomethingHappenedEventImposterBuilder Unsubscribed(
            Arg<EventHandler> criteria,
            Count count
        );
        ...
        ISomethingHappenedEventImposterBuilder Raised(
            Arg<object?> senderCriteria,
            Arg<EventArgs> argsCriteria,
            Count count
        );

        ISomethingHappenedEventImposterBuilder HandlerInvoked(
            Arg<EventHandler> handlerCriteria,
            Count count
        );
    }
    ...
    {
        IAsyncSomethingHappenedEventImposterBuilder Callback(
            Func<object?, EventArgs, Task> callback
        );

        Task<IAsyncSomethingHappenedEventImposterBuilder> RaiseAsync(
            object? sender,
            EventArgs args
        );

        IAsyncSomethingHappenedEventImposterBuilder Subscribed(
            Arg<Func<object?, EventArgs, Task>> criteria,
            Count count
        );

        IAsyncSomethingHappenedEventImposterBuilder Unsubscribed(
            Arg<Func<object?, EventArgs, Task>> criteria,
            Count count
        );

        IAsyncSomethingHappenedEventImposterBuilder OnSubscribe(
            Action<Func<object?, EventArgs, Task>> interceptor
        );

        IAsyncSomethingHappenedEventImposterBuilder OnUnsubscribe(
            Action<Func<object?, EventArgs, Task>> interceptor
        );

        IAsyncSomethingHappenedEventImposterBuilder Raised(
            Arg<object?> senderCriteria,
            Arg<EventArgs> argsCriteria,
            Count count
        );

        IAsyncSomethingHappenedEventImposterBuilder HandlerInvoked(
            Arg<Func<object?, EventArgs, Task>> handlerCriteria,
            Count count
        );
    }
    ...
    {
        private const string EventSignature =
            "Imposter.CodeGenerator.Tests.Features.EventImposterPoc.IEventImposterPocSut.SomethingHappened";
        ...
        public EventImposterPoc(ImposterMode mode = ImposterMode.Implicit)
        {
            ...
            _instance = new EventImposterPocSutInstance(
                _somethingHappened,
                _asyncSomethingHappened
            );
        }

        public ISomethingHappenedEventImposterBuilder SomethingHappened => _somethingHappened;

        public IAsyncSomethingHappenedEventImposterBuilder AsyncSomethingHappened =>
            _asyncSomethingHappened;

        public IEventImposterPocSut Instance() =>
            ((IHaveImposterInstance<IEventImposterPocSut>)this).Instance();
        ...
        private sealed class EventImposterPocSutInstance : IEventImposterPocSut
        {
            ...
            {
                add =>
                    _syncBuilder.Subscribe(value ?? throw new ArgumentNullException(nameof(value)));
                remove =>
                    _syncBuilder.Unsubscribe(
            ...
            {
                add =>
                    _asyncBuilder.Subscribe(
                        value ?? throw new ArgumentNullException(nameof(value))
                    );
                remove =>
                    _asyncBuilder.Unsubscribe(
                        value ?? throw new ArgumentNullException(nameof(value))
                    );
            }
        }

        private sealed class SomethingHappenedEventImposterBuilder
            : ISomethingHappenedEventImposterBuilder
        {
            private readonly ConcurrentQueue<EventHandler> _handlerOrder =
                new ConcurrentQueue<EventHandler>();
            private readonly ConcurrentDictionary<EventHandler, int> _handlerCounts =
                new ConcurrentDictionary<EventHandler, int>();
            private readonly ConcurrentQueue<EventHandler> _callbacks =
                new ConcurrentQueue<EventHandler>();
            private readonly ConcurrentQueue<(object? Sender, EventArgs Args)> _history =
                new ConcurrentQueue<(object? Sender, EventArgs Args)>();
            private readonly ConcurrentQueue<EventHandler> _subscribeHistory =
                new ConcurrentQueue<EventHandler>();
            private readonly ConcurrentQueue<EventHandler> _unsubscribeHistory =
                new ConcurrentQueue<EventHandler>();
            private readonly ConcurrentQueue<Action<EventHandler>> _subscribeInterceptors =
                new ConcurrentQueue<Action<EventHandler>>();
            private readonly ConcurrentQueue<Action<EventHandler>> _unsubscribeInterceptors =
                new ConcurrentQueue<Action<EventHandler>>();
            private readonly ConcurrentQueue<(
                ...
                new ConcurrentQueue<(EventHandler Handler, object? Sender, EventArgs Args)>();
            ...
            public ISomethingHappenedEventImposterBuilder Subscribed(
                Arg<EventHandler> criteria,
                Count count
            )
            {
            ...
            public ISomethingHappenedEventImposterBuilder Unsubscribed(
                Arg<EventHandler> criteria,
                Count count
            )
            {
                ...
                var actual = CountMatches(
                    _unsubscribeHistory,
                    handler => criteria.Matches(handler)
                );
                EnsureCountMatches(actual, count, "unsubscribed");
                return this;
            }

            public ISomethingHappenedEventImposterBuilder OnSubscribe(
                Action<EventHandler> interceptor
            )
            {
            ...
            public ISomethingHappenedEventImposterBuilder OnUnsubscribe(
                Action<EventHandler> interceptor
            )
            {
                ArgumentNullException.ThrowIfNull(interceptor);
            ...
            public ISomethingHappenedEventImposterBuilder Raised(
                Arg<object?> senderCriteria,
                Arg<EventArgs> argsCriteria,
                Count count
            )
            ...
            public ISomethingHappenedEventImposterBuilder HandlerInvoked(
                Arg<EventHandler> handlerCriteria,
                Count count
            )
            {
                ...
                var actual = CountMatches(
                    _handlerInvocations,
                    entry => handlerCriteria.Matches(entry.Handler)
                );
        ...
        private sealed class AsyncSomethingHappenedEventImposterBuilder
            : IAsyncSomethingHappenedEventImposterBuilder
        {
            private readonly ConcurrentQueue<Func<object?, EventArgs, Task>> _handlerOrder =
                new ConcurrentQueue<Func<object?, EventArgs, Task>>();
            private readonly ConcurrentDictionary<
                Func<object?, EventArgs, Task>,
                int
            > _handlerCounts = new ConcurrentDictionary<Func<object?, EventArgs, Task>, int>();
            private readonly ConcurrentQueue<Func<object?, EventArgs, Task>> _callbacks =
                new ConcurrentQueue<Func<object?, EventArgs, Task>>();
            private readonly ConcurrentQueue<(object? Sender, EventArgs Args)> _history =
                new ConcurrentQueue<(object? Sender, EventArgs Args)>();
            private readonly ConcurrentQueue<Func<object?, EventArgs, Task>> _subscribeHistory =
                new ConcurrentQueue<Func<object?, EventArgs, Task>>();
            private readonly ConcurrentQueue<Func<object?, EventArgs, Task>> _unsubscribeHistory =
                new ConcurrentQueue<Func<object?, EventArgs, Task>>();
            private readonly ConcurrentQueue<
                Action<Func<object?, EventArgs, Task>>
            > _subscribeInterceptors =
                new ConcurrentQueue<Action<Func<object?, EventArgs, Task>>>();
            private readonly ConcurrentQueue<
                Action<Func<object?, EventArgs, Task>>
            ...
            public IAsyncSomethingHappenedEventImposterBuilder Callback(
                Func<object?, EventArgs, Task> callback
            )
            {
                ArgumentNullException.ThrowIfNull(callback);
            ...
            public async Task<IAsyncSomethingHappenedEventImposterBuilder> RaiseAsync(
                object? sender,
                EventArgs args
            )
            {
            ...
            public IAsyncSomethingHappenedEventImposterBuilder Subscribed(
                Arg<Func<object?, EventArgs, Task>> criteria,
                Count count
            )
            {
            ...
            public IAsyncSomethingHappenedEventImposterBuilder Unsubscribed(
                Arg<Func<object?, EventArgs, Task>> criteria,
                Count count
            )
            {
            ...
            public IAsyncSomethingHappenedEventImposterBuilder OnSubscribe(
                Action<Func<object?, EventArgs, Task>> interceptor
            )
            {
                ArgumentNullException.ThrowIfNull(interceptor);
            ...
            public IAsyncSomethingHappenedEventImposterBuilder OnUnsubscribe(
                Action<Func<object?, EventArgs, Task>> interceptor
            )
            {
                ArgumentNullException.ThrowIfNull(interceptor);
            ...
            public IAsyncSomethingHappenedEventImposterBuilder Raised(
                Arg<object?> senderCriteria,
                Arg<EventArgs> argsCriteria,
                Count count
            )
            ...
            public IAsyncSomethingHappenedEventImposterBuilder HandlerInvoked(
                Arg<Func<object?, EventArgs, Task>> handlerCriteria,
                Count count
            )
            {
